...
git -C "$test_repo" tag -a v1.2.3-1 -m "Rejected suffix fixture"
expect_failure bash scripts/release
git -C "$test_repo" tag -d v1.2.3-1 >/dev/null
...
test "${result[1]}" = "v1.2.3"
rm "$test_repo/result"

git -C "$test_repo" tag -a v1.2.4 -m "Ambiguous release fixture"
expect_failure bash scripts/release

echo "KUBECTL_SERVICE_RELEASE_GATE_NEGATIVE_TEST_OK revision=${head_commit}"
